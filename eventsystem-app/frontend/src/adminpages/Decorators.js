...
function Decorators() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const addRes = await axios.post(
        "http://localhost:5000/decorators/addNewDecorator",
        formData,
        {
          headers: {
  ...
  const getDecorators = async () => {
    try {
      const resDecorators = await axios.get("http://localhost:5000/decorators/getDecorators")
      console.log("Decorators Data", resDecorators.data);
      const decoratorsData = resDecorators.data.data || [];
      setRecord(decoratorsData);
      doUpdate(!update);
    } catch (error) {
  ...
  const handleConfirmDelete = async () => {
    try {
      await axios.delete(
        `http://localhost:5000/decorators/deleteDecorator?DecoratorId=${selectedDeleteId}`
      );
